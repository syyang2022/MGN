...
def train():
    ...
    new_layers_optimizer = optim.SGD(new_params, momentum=0.9, weight_decay=5e-4, lr=0.003)
    old_layers_optimizer = optim.SGD(old_params, momentum=0.9, weight_decay=5e-4, lr=0.0003)
    ...
    for epoch in range(0, HyperParams['epoch']):
        ...
        # eval
        val_acc, test_loss = test(net, testloader, CELoss)
        ...
        with open(output_dir + '/results_train.txt', 'a') as file:
            file.write(
                'Iteration %d | train_acc = %.5f | train_loss = %.5f | test_acc = %.5f | test_loss = %.5f | best_acc = %.5f \n' % (
                epoch, train_acc, train_loss, val_acc, test_loss, max_val_acc))


def test(net, testloader, CELoss):
    net.eval()
    test_loss = 0
    correct_com = 0
    total = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        loss = CELoss(outputs_com, targets)
        test_loss += loss.item()
