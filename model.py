...
class FDM(nn.Module):
    ...
    def forward(self, f1, fm2):
        b, c, w1, h1 = f1.shape
        _, _, w2, h2 = fm2.shape
        fm1 = f1.view(b, c, -1)  # B*C*S
...
class FBSD(nn.Module):
    ...
    def forward(self, x):
        ...
        # # print(new_d1_from2.shape, new_d1_from3.shape)

        # f1, f2, f3 = self.fpn([fm1, fm2, fm3])
        # att1, att2, att3, a3, a4, a5 = self.apn([fm1, fm2, fm3])

        f1 = fm1
        f2 = F.interpolate(fm2, scale_factor=2)
        f3 = F.interpolate(fm3, scale_factor=4)

        att1 = self.conv_block1(f1)
        att2 = self.conv_block2(f2)
        att2 = (att2 + att1) / 2
        att3 = self.conv_block3(f3)
        att3 = (att3 + att2) / 2

        na31 = self.inter1(att3, att1)  # 3 1
        na23 = self.inter2(att2, att3)  # 2 3
        na12 = self.inter3(att1, att2)  # 1 2
